...
package Kernel::GenericInterface::Operation::User::UserAdd;
...
Kernel::GenericInterface::Operation::User::UserAdd - GenericInterface UserAdd Operation backend
...
sub new {
    ...
    $Self->{Config}    = $Kernel::OM->Get('Kernel::Config')->Get('GenericInterface::Operation::UserAdd');
    $Self->{Operation} = $Param{Operation};

    $Self->{DebugPrefix} = 'UserAdd';
...
sub Run {
    ...
    if ( $UserType eq 'Customer' ) {
        return $Self->ReturnError(
            ...
            ErrorMessage => $Self->{DebugPrefix} . ": CustomerUsers can't add users",
        );
    }
    ...
    if ( !$UserID ) {
        return $Self->ReturnError(
            ...
            ErrorMessage => $Self->{DebugPrefix} . ": Could not add user",
        );
    }
